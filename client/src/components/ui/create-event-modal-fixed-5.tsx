...
/**
 * Função utilitária para tentar abrir o seletor de data/hora nativo
 * Inclui fallback para casos onde showPicker não é suportado (como em iframes)
 */
const openDateTimePicker = (inputId: string) => {
  try {
    // Obter o elemento input
    const input = document.getElementById(inputId) as HTMLInputElement;
    if (!input) return;
    
    // Tenta abrir o seletor nativo
    input.showPicker();
  } catch (error) {
    console.error("Erro ao abrir seletor:", error);
    
    // Registrar o erro para análise futura
    logCreateEventError("Erro ao abrir seletor de data/hora", error);
    
    // Alternativa: focar no input geralmente abre o seletor nativo também
    const input = document.getElementById(inputId) as HTMLInputElement;
    if (input) {
      input.focus();
      // Em alguns navegadores, clicar no input após o foco pode ajudar
      input.click();
    }
  }
};
 ...
 */
const logCreateEventError = (message: string, error: any) => {
  console.error(`[CREATE-EVENT] ${message}:`, error);
  
  // Aqui poderia enviar para o sistema de logs ou analytics
  // No futuro, se necessário, integrar com o sistema de logs global
};
                    ...
                    <FormControl>
                      <div className="relative flex items-center">
                        ...
                        <Button 
                          ...
                          onClick={() => openDateTimePicker(`date-input-${field.name}`)}
                        >
